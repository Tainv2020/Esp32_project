...
void app_handle_data(esp_ble_gatts_cb_param_t *param);
...
void app_handle_data(esp_ble_gatts_cb_param_t *param)
{
    switch(param->write.len)
    {
        case 5:
        {
            if(param->write.value[0] == 't')
            {
                ESP_LOGI(GATTS_TAG, "Turn on Led 1");
            }
            break;
        }
        case 6:
        {
            if(param->write.value[0] == 'h')
            {
                ESP_LOGI(GATTS_TAG, "Turn on Led 2");
            }
            break;
        }
        default:
        {
            ESP_LOGI(GATTS_TAG, "Invalid data length");
            break;
        }
    }
}
